...
//
// Command line monitor program the Arduino Mega2560 development platform.
//
...
//----------------------------------------------------------------------
char readmem_int8(const uint32_t address, const int16_t offset=0) {
#ifdef HOSTTEST
...
//----------------------------------------------------------------------
int16_t readmem_int16(const uint32_t address, const int16_t offset=0) {
#ifdef HOSTTEST
    uint64_t address64 = address;
    // add an offset to generate a valid 64-bit address for test
    address64 += reinterpret_cast<uint64_t>(test_memory);
...
//----------------------------------------------------------------------
int32_t readmem_int32(const uint32_t address, const int16_t offset=0) {
#ifdef HOSTTEST
    uint64_t address64 = address;
    // add an offset to generate a valid 64-bit address for test
    address64 += reinterpret_cast<uint64_t>(test_memory);
...
bool to_number(const char* w) {
    ...
    char* endptr;
    int16_t v = (int16_t)strtol(w, &endptr, 0);
    dpush(v);
...
//----------------------------------------------------------------------
void writemem_int8(const uint32_t address, const int16_t value) {
#ifdef HOSTTEST
    uint64_t address64 = address;
    // add an offset to generate a valid 64-bit address for test
    address64 += reinterpret_cast<uint64_t>(test_memory);
...
//----------------------------------------------------------------------
void writemem_int16(const uint32_t address, const int16_t value) {
#ifdef HOSTTEST
    uint64_t address64 = address;
    // add an offset to generate a valid 64-bit address for test
    address64 += reinterpret_cast<uint64_t>(test_memory);
...
void dmem() {
    int16_t lines = dpop();
    int16_t address = dpop();
    address &= 0xfffffff0; // align to 16 bytes
    char buf[12];
    const int16_t addr_inc = 16;
    for(int16_t i=0; i<lines; ++i) {
        print_address(address);
        for(int16_t x=0; x<addr_inc; ++x) {
...
void setup() {
    // I/O init
#ifndef HOSTTEST
    Serial.begin(CONSPEED); // defined in the makfile
    ...
    Serial.println("Arduino Mega2560 Command Line Monitor");
